 ...
 *
 * Deposited logs are fetched over HyperRPC (fast, indexed) when HYPERRPC / ENVIO_API_KEY is set; every
 * state read + the totalDeposited() reconciliation stays on a real Gnosis RPC. The reconciliation guard
 * (event sums vs on-chain totalDeposited() at the cutoff block) is what makes HyperRPC-sourced logs safe.
 *
 * Usage:
 *   GNOSIS_RPC=<url> [HYPERRPC=<url> | ENVIO_API_KEY=<key>] \
 *       bun fetch-deposits.ts <depositContract> <basket.json> [out=build-config.json] \
...
// Resolve the RPC used for getLogs: prefer HyperRPC (fast, indexed) when configured, otherwise the real
// Gnosis RPC. State reads + reconciliation always use the real RPC (GNOSIS_RPC), never this.
...
if (import.meta.main) {
  ...
  console.log(
    `logs via ${logRpc.hyper ? 'HyperRPC' : 'public RPC'} (${new URL(logRpc.url).host}); state reads via ${new URL(RPC).host}`,
  )
  ...
  // Chunked log fetch — public RPCs cap getLogs ranges; a single 240k-block window would be silently
  // truncated, which would mis-build the root. Walk fixed windows so nothing is dropped. HyperRPC serves
  // the same eth_getLogs interface, so the chunking is harmless there and keeps one code path.
  const STEP = 10_000n
