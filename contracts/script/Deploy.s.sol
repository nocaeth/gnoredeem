...
///
/// @dev    Usage:
///           SAFE=0x...  DEADLINE=<unix>  OSGNO_RATE=<1e18-scaled> \
...
///         - OSGNO_RATE the osGNO->GNO rate (1e18) read from the rate provider AT THE GIP-151 SNAPSHOT
///                      BLOCK (00:00 UTC after the vote passes). Source it off-chain — this script cannot
///                      re-derive a historical value, it can only sanity-band it against the live rate:
///                        cast call 0x9B1b13afA6a57e54C03AD0428a4766C39707D272 "getRate()(uint256)" \
///                          --block <snapshotBlock> --rpc-url $RPC_GNOSIS
contract Deploy is Script {
    // Verified on Gnosis Chain (chainid 100) this session — see docs/contract-expert-review.md.
